...
- `build_astrbot_428_mem0.sh` 只将 Dockerfile、依赖清单和已审核的 provider 补丁脚本打包送入构建器，不发送数据、配置或备份目录；构建后验证浏览器、依赖、源文件和隔离 Mem0 读写。
- `disable_openai_sdk_retries.py` 在镜像构建时增加 provider 级 `openai_sdk_max_retries` 配置。线上仅对不稳定的 DeepSeek source 设为 `0`，其余 provider 默认仍为 `2`；DeepSeek 失败后由 AstrBot 的 `agent_runner.config.model.request_max_retries=1` 立即触发备用模型，避免上游 `502` 的 `Retry-After` 让微信长时间静默。
...
2. 将 Dockerfile 放到 `/root/astrbot/Dockerfile.astrbot-goofish`，其余运行脚本、`disable_openai_sdk_retries.py` 和 `runtime-python-requirements.txt` 放到 `/root/astrbot`，另复制 `../test_mem0_takeover.py`。所有文件使用 UTF-8 与 LF。
