...
function renderConfigForm(cfg) {
  ...
  var groups = [
    ...
    {title:'桥接设置', fields:[
      ...
      {key:'allowed_groups', label:'允许的群（逗号分隔，留空不限）', type:'text', ph:'群 session ID 或完整群名'},
    ]},
    {title:'图片描述', fields:[
      {key:'image_caption_provider', label:'描述服务', type:'select', opts:[{v:'ollama',l:'Ollama（qlsg/本地）'},{v:'openai',l:'OpenAI 兼容'}]},
      {key:'image_caption_model', label:'模型名', type:'text', ph:'qwen2.5vl:3b（需 vision 能力）'},
      ...
      {key:'image_caption_fallbacks', label:'备用描述服务（JSON，可选）', type:'textarea', ph:'[{"api_base":"...","model":"...","api_key":"..."}]'},
    ]},
    {title:'Ollama（使用本地模式时）', fields:[
      {key:'ollama_base_url', label:'Ollama 地址（qlsg 转发）', type:'text', ph:'http://127.0.0.1:61000'},
  ...
  groups.forEach(function(g){
    ...
    g.fields.forEach(function(f){
      ...
      if (f.key === 'image_caption_fallbacks' && Array.isArray(val)) val = JSON.stringify(val, null, 2);
...
function saveConfig() {
  ...
  fields.forEach(function(el){
    ...
    // 逗号分隔字段转数组
    if (key === 'bot_nicknames' || key === 'allowed_groups') val = val ? val.split(/[,，]\\s*/).filter(Boolean) : [];
    if (key === 'image_caption_fallbacks' && val) {
      try { val = JSON.parse(val); } catch (e) { showMsg('❌ 备用服务 JSON 格式错误'); return; }
    }
    data[key] = val;
  });
    ...
    def do_GET(self):
        if self.path == "/status":
            ...
            self.send_json({
                ...
                "uia_ready": bool(getattr(state, "sender_instance", None) and
                                   getattr(state.sender_instance, "_ready", False)),
    ...
    def do_POST(self):
        if self.path == "/start":
            ...
            try:
                ...
                if sender is None:
                    self.send_json({"ok": False, "error": "sender unavailable"}, 503)
                    return
                sent = sender.send_text(contact, text)
                self.send_json({"ok": bool(sent)})
            except Exception as e:
                log.error(f"[Web] 测试发送异常: {type(e).__name__}")
                self.send_json({"ok": False, "error": "test send failed"}, 500)
        elif self.path == "/mode":
